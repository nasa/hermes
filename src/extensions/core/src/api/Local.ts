...
class Local extends Rpc.Client {
    ...
    static async startTask(context: vscode.ExtensionContext): Promise<vscode.TaskExecution> {
        ...
        const bindSettings = Settings.hostBind();
        const bindType = bindSettings.bindType ?? "unix";
        const bind = bindType == "unix" ? tmp.tmpNameSync({ template: "hermes-transport-XXXXXX.sock" }) : "localhost:6880";
        ...
        const task = new vscode.Task(
            ...
            } as LocalTaskDefinition, // cast provides type checking
            vscode.TaskScope.Workspace,
            "Hermes",
            "hermes"
        );
