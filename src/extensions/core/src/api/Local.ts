...
export class LocalBackendProvider implements BackendProvider<LocalBackendExecution | LocalTaskDefinition> {
    type = "local";
    title = "Local";
    ...
    async promptForState(): Promise<LocalBackendExecution | LocalTaskDefinition | null> {
        return await pickLocalDialog();
    }

    async provideBackendApi(
        state: LocalBackendExecution | LocalTaskDefinition,
        ...
        if (state instanceof LocalBackendExecution) {
            ...
            const execution = await Local.startTask(state, context);
...
async function pickLocalDialog(current?: LocalTaskDefinition): Promise<LocalTaskDefinition | null> {
    const workspaceDir = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceDir) {
        throw new Error("Cannot run local backend outside of workspace");
    }
    ...
    const selected = quickPick.items.find((v) => {
        if (v && current) {
            return current.bind === v.bind;
        } else {
            return false;
        }
    });

    quickPick.activeItems = [selected ? selected : quickPick.items[0]];
    quickPick.show();

    const subs: vscode.Disposable[] = [];
    const pick = await new Promise<LocalTaskDefinition | null>((resolve) => {
        subs.push(
            quickPick.onDidHide(() => {
                resolve(null);
            }),
